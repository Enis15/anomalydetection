...
if __name__ == '__main__':
    ...
    metrics_df.to_csv('../results/performance/Metrics(DS4).csv', index=False)
    _logger.info('The evaluation results are saved to CSV file.')

    unsupervised_metrics_df.to_csv('../results/performance/Unsupervised_Metrics(DS4).csv', index=False)
    ...
    plt.savefig('../results/performance/ROC_AUC_vs_Runtime(DS4).png', bbox_inches='tight')
    plt.show()
    ...
    plt.savefig('../results/performance/F1_Score_vs_Runtime(DS4).png', bbox_inches='tight')
    plt.show()
