...
from utils.paramet_tune import paramet_tune, Catboost_tuner, LOF_tuner, RandomForest_tuner, KNN_tuner, DBSCAN_tuner
...
if __name__ == '__main__':
    ...
    try:
        # MODEL K-NEAREST NEIGHBORS (KNN)
        _logger.info('Starting KNN-tuner Evaluation')
        # Tune the KNN model to get the best hyperparameters
        knn_tuner = KNN_tuner(X_train, X_test, y_train, y_test)
        best_knn = knn_tuner.tune_model()
        print(best_knn)  # Get the results of parameter tuning
        _logger.info(f'Best K-Nearest Neighbor Model(tuned): {best_knn}')
        neighbor = int(best_knn['n_neighbors']) # Save the value of k
        # Evaluate the KNN model using the best parameters
        knn_roc, knn_f1, knn_time = model_knn(X_train, X_test, y_train, y_test, neighbor)
        append_metrics('KNN-tuner', neighbor, knn_roc, knn_f1, knn_time)
        _logger.info(f'KNN Evaluation(tuned): ROC AUC SCORE={knn_roc}, F1 SCORE={knn_f1}, Runtime={knn_time}')
    except Exception as e:
        _logger.error(f'Error evaluating KNN model:{e}')
    ...
    metrics_df.to_csv('./Metrics(DS2).csv', index=False)
    _logger.info('The evaluation results are saved to CSV file.')

metrics_df = pd.read_csv('../../results/Metrics(DS2).csv')
...
plt.savefig('./ROC_AUC_vs_Runtime(DS2).png', bbox_inches='tight')
plt.show()
...
plt.savefig('./F1_Score_vs_Runtime.png(DS2)', bbox_inches='tight')
plt.show()